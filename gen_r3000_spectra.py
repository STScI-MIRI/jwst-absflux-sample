...
def save_rebin_file(in_file, out_file, target_waveres):
    ...
    rb_table = Table()
    rb_table["WAVELENGTH"] = Column(
        trunc_rebin(ctable["WAVELENGTH"] * 1e-4, rebin_fac),
    ...
    rb_table["FLUX"] = Column(
        trunc_rebin(ctable["FLUX"], rebin_fac), description="flux"
    )
    if "CONTINUUM" in rb_table.colnames:
        rb_table["CONTINUUM"] = Column(
            trunc_rebin(ctable["CONTINUUM"], rebin_fac), description="continuum"
        )
...
if __name__ == "__main__":
    ...
    rfac = 50
    for cname in np.concatenate((astarnames, gstarnames, hotstarnames)):
        print(cname)
